...
then
  ...
  mysql -u root -p$DB_ROOT_PARENT_PW -h $MYSQL_HOST -e 'update mysql.user set authentication_string=PASSWORD("$MYSQL_ROOT_PASSWORD") where user="root" limit 1;flush privileges;'
